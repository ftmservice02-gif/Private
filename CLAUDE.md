...
| `projects` | `id UUID`, `title`, `total_days_budget`, `created_by → users`, plus the "New project" intake form's `project_code`, `short_name`, `fiscal_year`, `project_type`, `project_value`, `system_function`, `key_components`, `organization_id → organizations`, `owner_id → project_owners` | One row per project/board. |
| `organizations`, `project_owners` | `id UUID`, `name TEXT UNIQUE` | Lookup tables behind the intake form's ชื่อหน่วยงาน / ผู้รับผิดชอบโครงการ pickers — deliberately separate from `users` (a name here isn't necessarily a workspace member with a login). |
| `project_contacts`, `project_product_registrations`, `project_sites` | `id UUID`, `project_id → projects`, `name`/`location`, (`email` on `project_contacts` only), `sort_order` | The intake form's Section 3 sub-lists (ผู้ติดต่อประสานงาน / ลงทะเบียนผลิตภัณฑ์ / สถานที่ติดตั้ง) — simple named lists scoped to one project, added/removed one row at a time via their own small endpoints. |
